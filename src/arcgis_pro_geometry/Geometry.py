...
class Geometry:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        This method is called when the 'with' statement is exited. It deletes the geometry instance attributes,
        freeing up the memory it was using.
    ...
    def cut(self, cut_polyline, out_fc):
        ...
        if cut_polyline_geom.shape_type != 'polyline':
            arcpy.AddError(f'The cutting polyline has shape type {cut_polyline_geom.shape_type}!')
        ...
        if self.shape_type == 'point':
            ...
            arcpy.CopyFeatures_management(cut, r'memory\out_fc')
            arcpy.MultipartToSinglepart_management(r'memory\out_fc', out_fc)

            del cut_polyline_geom
            arcpy.Delete_management(r'memory\out_fc')
    ...
    def distance_lines(self, other_fc, out_fc, overwrite_out_fc=True):
        ...
        if not arcpy.Exists(out_fc) or overwrite_out_fc is True:
            ...
            if self.shape_type != "point":
                arcpy.AddError(f"Only point features are accepted, not '{self.shape_type}'")
                exit()

            elif geom_other.shape_type in ['multipoint', 'multipatch', 'dimension', 'annotation']:
                message_gt = f"The first feature class has geometry type '{self.shape_type}'. " \
                             f"Please only use 'polygon', 'polyline' oder 'point'!"
                arcpy.AddError(message_gt)
                exit()

            if any([x.isMultipart for x in self.shape]):
                message_1st_fc_mp = "The first feature class is not allowed to have multipart!"
                arcpy.AddError(message_1st_fc_mp)
                exit()

            elif any([x.isMultipart for x in geom_other.shape]):
                message_2nd_fc_mp = "The seconds feature class is not allowed to have multipart!"
                ...
                # arcpy.DefineProjection_management(out_fc, self.spatial_reference)  # xxx
        ...
        def converge_to_the_center(polygon, centroids_list, distances_list):
            ...
            while True:
                try:
                    ...
                    if min_dist <= accuracy or shape.pointCount == 2:
                        centroids_list.append(point_geom)
                        distances_list.append(dist)
                        break

                    polygon = polygon.buffer(float(f"-{min_dist}"))
    ...
    def numerate(self, sort_by='top_left', field_name='id'):
        ...
        if self.shape_type == 'point':
            ...
            oid_x_y_list = [row for row in arcpy.da.SearchCursor(self.feature, ['SHAPE@X', 'SHAPE@Y', 'OID@'])]
    ...
    def polyline_to_polygon(self, out_polygon):
        ...
        for array_type in array_types:
            json_str = json_str.replace(array_type, array_types[array_type])
