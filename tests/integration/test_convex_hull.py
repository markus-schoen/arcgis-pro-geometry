# SCRIPT --------------------------------------------------------------------------------------------------------------
# Integration test: convex_hull method
...
def test_convex_hull(fc_name, results_gdb):
    ...
    convex_hull_name = f'{fc_name}_convex_hull'
    convex_hull = os.path.join(dataset_results, convex_hull_name)
    convex_hull_test = os.path.join(results_gdb, convex_hull_name)

    try:
        geom = Geometry(fc)
        geom.convex_hull(convex_hull_test)

        result = arcpy.FeatureCompare_management(
            convex_hull, convex_hull_test, 'OBJECTID', continue_compare='CONTINUE_COMPARE'
        )

        assert result.getOutput(1) == 'true'

    finally:
        arcpy.Delete_management(convex_hull_test)
# ---------------------------------------------------------------------------------------------------------------------
