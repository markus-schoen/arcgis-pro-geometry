# SCRIPT --------------------------------------------------------------------------------------------------------------
# Integration test: hull_rectangle method
...
def test_hull_rectangle(fc_name, results_gdb):
    ...
    hull_rectangle_name = f'{fc_name}_hull_rectangle'
    hull_rectangle = os.path.join(dataset_results, hull_rectangle_name)
    hull_rectangle_test = os.path.join(results_gdb, hull_rectangle_name)

    try:
        geom = Geometry(fc)
        geom.hull_rectangle(hull_rectangle_test)

        result = arcpy.FeatureCompare_management(
            hull_rectangle, hull_rectangle_test, 'OBJECTID', continue_compare='CONTINUE_COMPARE'
        )

        assert result.getOutput(1) == 'true'

    finally:
        arcpy.Delete_management(hull_rectangle_test)
# ---------------------------------------------------------------------------------------------------------------------
